...
public class ResourceManagerCarImpl
	...
	public static void main(String args[]) {
        ...
        if (args.length == 1) {
	 	...
	 	try {
			...
			ResourceManagerCarImpl obj = new ResourceManagerCarImpl();
	 ...
	 public ResourceManagerCarImpl() throws RemoteException {
		...
		throws RemoteException
	{
		updateTTL(xid);
		ArrayList<Object> carHistory = carHistories.get(new Integer(xid));
        carHistory.add(((Car)readData( id, Car.getKey(location) )).clone());
        carHistories.put(new Integer(xid), carHistory);
		boolean del = deleteItem(id, Car.getKey(location));
		shadowData();
		return del;
	}
...
//	
		...
		throws RemoteException
	{
        ...
        carHistory.add(((Car)readData( id, Car.getKey(location) )).clone());
        carHistories.put(new Integer(xid), carHistory);
		Customer res = reserveItem(id, cust, Car.getKey(location), location);
		shadowData();
		return res;
	}
     	...
     	if (carHistories.containsKey(new Integer(xid))) {
            ...
            for (int i = carHistory.size() - 1; i >= 0; i--) {
                if (carHistory.get(i) instanceof String) {
                    String addedLocation = (String)carHistory.get(i);
                    removeData(0, Car.getKey(addedLocation));
                } else {
                    Car oldCar = (Car)(carHistory.get(i));
                    writeData(0, oldCar.getKey(), oldCar);
                }
            }
            shadowData();
